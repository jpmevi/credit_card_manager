...
export class init1698544980751 implements MigrationInterface {
    name = 'init1698544980751'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`review\` (\`id\` int NOT NULL AUTO_INCREMENT, \`comment\` text NOT NULL, \`rate\` int UNSIGNED NOT NULL, \`username\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`user\` (\`username\` varchar(255) NOT NULL, \`cui\` varchar(45) NOT NULL, \`email\` varchar(45) NOT NULL, \`first_name\` varchar(255) NOT NULL, \`last_name\` varchar(255) NOT NULL, \`dob\` date NOT NULL, \`pin\` varchar(45) NOT NULL, \`role\` enum ('administrator', 'costumer') NOT NULL DEFAULT 'costumer', \`notify_me\` tinyint NOT NULL DEFAULT 1, \`reminder\` tinyint NOT NULL DEFAULT 1, \`create_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), PRIMARY KEY (\`username\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`transaction\` (\`id\` int NOT NULL AUTO_INCREMENT, \`amount\` decimal(10,2) NOT NULL, \`old_balance\` decimal(10,2) NOT NULL, \`current_balance\` decimal(10,2) NOT NULL, \`type\` enum ('increase', 'decrease') NOT NULL, \`create_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`doe\` date NOT NULL, \`log\` text NOT NULL, \`date\` date NOT NULL, \`account_number\` varchar(50) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`account\` (\`number\` varchar(50) NOT NULL, \`doe\` date NOT NULL, \`cvv\` varchar(255) NOT NULL, \`status\` enum ('enabled', 'disabled', 'deleted') NOT NULL DEFAULT 'enabled', \`balance\` decimal(10,2) NOT NULL, \`limit\` decimal(10,2) NOT NULL, \`rejections\` int UNSIGNED NOT NULL, \`username\` varchar(255) NOT NULL, \`id_account_type\` int NOT NULL, UNIQUE INDEX \`REL_7461b5f6501d561926cb0e9f68\` (\`id_account_type\`), PRIMARY KEY (\`number\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`DROP INDEX \`REL_7461b5f6501d561926cb0e9f68\` ON \`account\``);
